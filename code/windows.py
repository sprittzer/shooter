...
class Level:
    ...
    def setup_level(self, layout):
        ...
        self.box.add(Box())
        self.gun.add(MachineGun(self.player.sprite))
        self.fire.add(Fire())
        ...
        for enemy in self.enemies.sprites():
            if pygame.sprite.collide_mask(fire, enemy):
                if enemy.__class__.__name__ == 'Enemy':
                    ...
                    enemy.rect = enemy.image.get_rect(topleft = (113, -25))
                    if enemy.speed < 0:
                        enemy.speed = -1.5
                    else:
                        enemy.speed = 1.5
                else:
                    enemy.sprites = load_sprite_sheets('angry_enemy2', 20, 20)
                    if enemy.speed < 0:
                        enemy.speed = -1
                    else:
                        enemy.speed = 1
                    enemy.rect = enemy.image.get_rect(topleft = (110, -25))
                enemy.visible = False
                enemy.frame_index = 0
    ...
    def adding_enemies(self):
        if random() < self.percent_of_enemy:
            for _ in range(randint(1, 3)):
                ...
                if random() > 0.2:
                    pos = (117, randrange(-250, -25)) if direction == 'right' else (110, randrange(-250, -25))
                    self.enemies.add(Enemy(direction, pos))
                else:
                    pos = (110, randrange(-250, -25)) if direction == 'right' else (110, randrange(-250, -25))
                    self.enemies.add(BigEnemy(direction, pos))

    def check_enemy_visible(self):
        for enemy in self.enemies:
            if self.display_surface.get_rect().contains(enemy.rect) and enemy.rect.y > 20:
                enemy.visible = True

    def run(self, main_screen):
        if self.gun.sprite.long_press:
            self.get_input()
        if len(self.enemies.sprites()) < 3:
            self.adding_enemies()
