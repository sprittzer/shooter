...
class BigEnemy(pygame.sprite.Sprite):
    def __init__(self, direction, pos):
        super().__init__()
        self.sprites = load_sprite_sheets('enemy2', 20, 20)
        self.speed = 0.8
        ...
        self.damage = 0
        self.visible = False
        pygame.time.set_timer(SPAWN_ENEMY, choice([3000, 4000, 5000]))
...
class Bullet(pygame.sprite.Sprite):
    def __init__(self, coords, x, y, image):
        super().__init__()
        self.image = image
...
class Pistol(Gun):
    ...
    def new_bullet(self):
        if self.player.facing_left:
            ...
            x += 3
            bullet = Bullet((x, y), -self.speed_x, self.speed_y, self.image_b)
        else:
...
class DualPistols(Gun):
    ...
    def new_bullet(self):
        ...
        x += 3
        y -= 1
        bullet1 = Bullet((x, y), -self.speed_x, self.speed_y, self.image_b)
        x, y = self.rect.midright
        x -= 3
        y -= 1
        bullet2 = Bullet((x, y), self.speed_x, self.speed_y, self.image_b)
...
class Revolver(Gun):
    ...
    def new_bullet(self):
        if self.player.facing_left:
            bullet = Bullet(self.player.rect.midleft, -self.speed_x, self.speed_y, self.image_b)
        else:
            bullet = Bullet(self.player.rect.midright, self.speed_x, self.speed_y, self.image_b)
        return [bullet]
